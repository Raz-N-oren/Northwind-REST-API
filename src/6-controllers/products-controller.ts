...
router.get("/products/:id([0-9]+)", async (request: Request, response: Response, next: NextFunction) => {
    try {
        ...
        const product = await productsLogic.getOneProducts(id);
...
router.get("/products-cheaper-than/:minPrice([0-9]+)", async (request: Request, response: Response, next: NextFunction) => {
    try {
